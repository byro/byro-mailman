...
class MailmanMemberForm(forms.Form):
    ...
    def __init__(self, *args, member, **kwargs):
        ...
        names = MailingList.objects.exclude(subscribers__member=member).values_list(
            "name", flat=True
        )
...
class MemberAdd(MemberLists):
    def post(self, request, pk):
        ...
        if not form.is_valid():
            ...
            return redirect(
                reverse(
                    "plugins:byro_mailman:members.mailman.lists",
                    kwargs={"pk": self.kwargs["pk"]},
                )
            )
        try:
            mailing_list = MailingList.objects.filter(
                name=form.cleaned_data["mailinglist"]
            ).first()
            mailing_list.add(self.get_object())
            messages.success(request, _("Member added to mailing list."))
        except Exception as e:
            messages.error(
                request, _("Error adding the member to the mailing list: ") + str(e)
            )
        return redirect(
            reverse(
                "plugins:byro_mailman:members.mailman.lists",
...
class MemberRemove(MemberLists):
    def get(self, request, pk, list_id):
        ...
        try:
            ...
            messages.error(
                request, _("Error removing the member from the mailing list: ") + str(e)
            )
        return redirect(
            reverse(
                "plugins:byro_mailman:members.mailman.lists",
...
class MailmanSync(View):
    def dispatch(self, *args, **kwargs):
        ...
        if "list_id" in self.request.GET:
            messages.success(
                self.request, _("The mailing list has been synced. ") + stats
            )
        else:
            messages.success(
                self.request, _("The mailing lists have been synced. ") + stats
            )
